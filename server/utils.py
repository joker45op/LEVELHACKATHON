...
def generate_birth_chart_svg(name, year, month, day, hour=12, minute=0, lng=0.0, lat=0.0,city="Mumbai", tz_str="Asia/Tokyo" , output_dir="./server/imgs"):
    """
...
def json_to_multiline_text(data):
    result = []
    
    def recursive_format(key, value, indent=0):
        line = "  " * indent + f"{key}: {value}"
        result.append(line)
        
        if isinstance(value, dict):
            for sub_key, sub_value in value.items():
                recursive_format(sub_key, sub_value, indent + 1)
        elif isinstance(value, list):
            for item in value:
                recursive_format("-", item, indent + 1)

    for key, value in data.items():
        recursive_format(key, value)
